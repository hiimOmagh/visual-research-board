# RUNBOOK v2.1.11 — Dependency Audit Resolution + Safe Upgrade Lock

## Purpose

This runbook locks the v2.1.11 release verification path after the dependency audit safe-upgrade lock was added.
...
[release-verify] passed for v2.1.11
Verification Report Freshness Lock + Warning Suppression checks passed for v2.1.11.
Nested Verification Warning Silence + Final Freshness Recheck checks passed for v2.1.11.
```
...
- `artifacts/full-qa-gate-report.json` has `appVersion` or equivalent version marker for `2.1.11`
- `artifacts/full-qa-gate-report.json` has passed status
- `artifacts/release-verify-report.json` has `appVersion` or equivalent version marker for `2.1.11`
...
v2.1.11 is accepted only when this command passes:
