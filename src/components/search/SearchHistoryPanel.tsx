...
export function SearchHistoryPanel({ history }: { history: SearchHistoryEntry[] }) {
  if (history.length === 0) {
    return (
      <section className="rounded-3xl border border-dashed border-white/15 bg-white/[0.03] p-5 text-sm text-slate-400">
        Search history will be stored inside the active project after the first search.
      </section>
    );
  }
        ...
        {history.map((entry) => (
          <article key={entry.id} className="rounded-2xl border border-white/10 bg-black/20 p-4">
            ...
            <p className="mt-2 text-xs text-slate-400">
              {entry.query_count} queries · {entry.duplicate_count} duplicates removed
            </p>
