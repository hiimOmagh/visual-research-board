...
async function executeProviderRuns(params: {
  ...
  return Promise.all([
    ...
    runProvider({ provider: "rijksmuseum", enabled: runtimeToggles.rijksmuseum && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Rijksmuseum was intentionally skipped." : "Rijksmuseum is disabled by default until its free key is configured.", missingEnv: runtimeToggles.rijksmuseum && !process.env.RIJKSMUSEUM_API_KEY ? "RIJKSMUSEUM_API_KEY" : undefined, missingKeyMessage: "Rijksmuseum is free-key only in this app. Add RIJKSMUSEUM_API_KEY or leave the provider off.", endpointSample: ["www.rijksmuseum.nl/api/en/collection"], plan: searchPlan, run: () => searchRijksmuseum(searchPlan) }),
    ...
    runProvider({ provider: "nypl", enabled: runtimeToggles.nypl && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; NYPL was intentionally skipped." : "NYPL Digital Collections is disabled by default until its free key is configured.", missingEnv: runtimeToggles.nypl && !process.env.NYPL_API_KEY ? "NYPL_API_KEY" : undefined, missingKeyMessage: "NYPL is free-key only in this app. Add NYPL_API_KEY or leave the provider off.", endpointSample: ["api.repo.nypl.org/api/v1/items/search.json"], plan: searchPlan, run: () => searchNyplDigitalCollections(searchPlan) }),
