...
} from "@/lib/providers/museum-open-access";
import { searchPexels, searchPixabay, searchUnsplash } from "@/lib/providers/stock-illustrative";
...
import { detectPublicSecretEnvKeys, getProviderKeyPresenceFromEnv, missingProviderSecretEnv } from "@/lib/provider-key-security";
...
async function executeProviderRuns(params: {
  ...
  return Promise.all([
    ...
    runProvider({ provider: "smithsonian", enabled: runtimeToggles.smithsonian && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Smithsonian was intentionally skipped." : "Smithsonian is disabled or this mode did not request museum/open-access targets.", missingEnv: runtimeToggles.smithsonian ? missingProviderSecretEnv("smithsonian") : undefined, missingKeyMessage: "Smithsonian Open Access is free-key only. Add SMITHSONIAN_API_KEY or leave the provider off.", endpointSample: ["api.si.edu/openaccess/api/v1.0/search"], plan: searchPlan, run: () => searchSmithsonianOpenAccess(searchPlan) }),
    runProvider({ provider: "europeana", enabled: runtimeToggles.europeana && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Europeana was intentionally skipped." : "Europeana is disabled or this mode did not request cultural heritage targets.", missingEnv: runtimeToggles.europeana ? missingProviderSecretEnv("europeana") : undefined, missingKeyMessage: "Europeana is free-key only. Add EUROPEANA_API_KEY or leave the provider off.", endpointSample: ["api.europeana.eu/record/v2/search.json"], plan: searchPlan, run: () => searchEuropeana(searchPlan) }),
    runProvider({ provider: "met", enabled: runtimeToggles.met && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Met Museum was intentionally skipped." : "Met Museum is disabled or this mode did not request museum/open-access targets.", endpointSample: ["collectionapi.metmuseum.org/public/collection/v1"], plan: searchPlan, run: () => searchMetMuseum(searchPlan) }),
    runProvider({ provider: "artic", enabled: runtimeToggles.artic && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Art Institute of Chicago was intentionally skipped." : "Art Institute of Chicago is disabled or this mode did not request museum/open-access targets.", endpointSample: ["api.artic.edu/api/v1/artworks/search"], plan: searchPlan, run: () => searchArtInstituteChicago(searchPlan) }),
    runProvider({ provider: "cleveland_museum", enabled: runtimeToggles.cleveland_museum && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Cleveland Museum was intentionally skipped." : "Cleveland Museum is disabled or this mode did not request museum/open-access targets.", endpointSample: ["openaccess-api.clevelandart.org/api/artworks"], plan: searchPlan, run: () => searchClevelandMuseum(searchPlan) }),
    runProvider({ provider: "rijksmuseum", enabled: runtimeToggles.rijksmuseum && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Rijksmuseum was intentionally skipped." : "Rijksmuseum is disabled by default until its free key is configured.", missingEnv: runtimeToggles.rijksmuseum ? missingProviderSecretEnv("rijksmuseum") : undefined, missingKeyMessage: "Rijksmuseum is free-key only in this app. Add RIJKSMUSEUM_API_KEY or leave the provider off.", endpointSample: ["www.rijksmuseum.nl/api/en/collection"], plan: searchPlan, run: () => searchRijksmuseum(searchPlan) }),
    runProvider({ provider: "wellcome", enabled: runtimeToggles.wellcome && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Wellcome Collection was intentionally skipped." : "Wellcome Collection is disabled or this mode did not request museum/open-access targets.", endpointSample: ["api.wellcomecollection.org/catalogue/v2/works"], plan: searchPlan, run: () => searchWellcomeCollection(searchPlan) }),
    runProvider({ provider: "bhl", enabled: runtimeToggles.bhl && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Biodiversity Heritage Library was intentionally skipped." : "Biodiversity Heritage Library is disabled or this mode did not request archive/science targets.", endpointSample: ["biodiversitylibrary.org/api3"], plan: searchPlan, run: () => searchBiodiversityHeritageLibrary(searchPlan) }),
    runProvider({ provider: "gallica", enabled: runtimeToggles.gallica && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Gallica/BnF was intentionally skipped." : "Gallica/BnF is disabled or this mode did not request archive/museum targets.", endpointSample: ["gallica.bnf.fr/SRU"], plan: searchPlan, run: () => searchGallicaBnf(searchPlan) }),
    runProvider({ provider: "nypl", enabled: runtimeToggles.nypl && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; NYPL was intentionally skipped." : "NYPL Digital Collections is disabled by default until its free key is configured.", missingEnv: runtimeToggles.nypl ? missingProviderSecretEnv("nypl") : undefined, missingKeyMessage: "NYPL is free-key only in this app. Add NYPL_API_KEY or leave the provider off.", endpointSample: ["api.repo.nypl.org/api/v1/items/search.json"], plan: searchPlan, run: () => searchNyplDigitalCollections(searchPlan) }),
    runProvider({ provider: "nara", enabled: runtimeToggles.nara && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image")), skippedMessage: mockOnly ? "Mock-only safe mode is active; NARA was intentionally skipped." : "NARA is disabled or this mode did not request archive/image targets.", endpointSample: ["catalog.archives.gov/api/v1"], plan: searchPlan, run: () => searchNaraCatalog(searchPlan) }),
    runProvider({ provider: "dpla", enabled: runtimeToggles.dpla && (searchPlan.source_targets.includes("archive") || searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("commons")), skippedMessage: mockOnly ? "Mock-only safe mode is active; DPLA was intentionally skipped." : "DPLA is disabled by default until its free key is configured.", missingEnv: runtimeToggles.dpla ? missingProviderSecretEnv("dpla") : undefined, missingKeyMessage: "DPLA is free-key only in this app. Add DPLA_API_KEY or leave the provider off.", endpointSample: ["api.dp.la/v2/items"], plan: searchPlan, run: () => searchDpla(searchPlan) }),
    runProvider({ provider: "pixabay", enabled: runtimeToggles.pixabay && searchPlan.source_targets.includes("image"), skippedMessage: mockOnly ? "Mock-only safe mode is active; Pixabay was intentionally skipped." : "Pixabay is disabled by default; enable it only for stock/illustrative candidates.", missingEnv: runtimeToggles.pixabay ? missingProviderSecretEnv("pixabay") : undefined, missingKeyMessage: "Pixabay is a free-key stock/illustrative provider. Add PIXABAY_API_KEY or leave it disabled.", endpointSample: ["pixabay.com/api"], plan: searchPlan, run: () => searchPixabay(searchPlan) }),
    runProvider({ provider: "pexels", enabled: runtimeToggles.pexels && searchPlan.source_targets.includes("image"), skippedMessage: mockOnly ? "Mock-only safe mode is active; Pexels was intentionally skipped." : "Pexels is disabled by default; enable it only for stock/illustrative candidates.", missingEnv: runtimeToggles.pexels ? missingProviderSecretEnv("pexels") : undefined, missingKeyMessage: "Pexels is a free-key stock/illustrative provider. Add PEXELS_API_KEY or leave it disabled.", endpointSample: ["api.pexels.com/v1/search"], plan: searchPlan, run: () => searchPexels(searchPlan) }),
    runProvider({ provider: "unsplash", enabled: runtimeToggles.unsplash && searchPlan.source_targets.includes("image"), skippedMessage: mockOnly ? "Mock-only safe mode is active; Unsplash was intentionally skipped." : "Unsplash is disabled by default; enable it only for stock/illustrative candidates.", missingEnv: runtimeToggles.unsplash ? missingProviderSecretEnv("unsplash") : undefined, missingKeyMessage: "Unsplash is a free-key stock/illustrative provider. Add UNSPLASH_ACCESS_KEY or leave it disabled.", endpointSample: ["api.unsplash.com/search/photos"], plan: searchPlan, run: () => searchUnsplash(searchPlan) }),
    runProvider({ provider: "brave", enabled: runtimeToggles.brave && (searchPlan.source_targets.includes("image") || searchPlan.source_targets.includes("web")), skippedMessage: mockOnly ? "Mock-only safe mode is active; Brave was intentionally skipped." : "Brave is optional and disabled by default for the free-only workflow.", missingEnv: runtimeToggles.brave ? missingProviderSecretEnv("brave") : undefined, missingKeyMessage: "Brave Search is optional and requires BRAVE_SEARCH_API_KEY. It is not part of the free-only core.", endpointSample: ["api.search.brave.com/res/v1/images/search", "api.search.brave.com/res/v1/web/search"], plan: searchPlan, run: async () => {
...
export async function POST(request: Request) {
  ...
  return NextResponse.json({
    ...
    diagnostics: {
      ...
      key_security: runtimeReport.key_security
    }
  });
}
