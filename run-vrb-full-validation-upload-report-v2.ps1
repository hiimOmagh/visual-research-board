...
Run-Step -Name "Package scripts snapshot" -Command 'node -e "const p=require(''./package.json''); const scripts=Object.keys(p.scripts || {}).sort(); console.log(JSON.stringify({version:p.version,scripts:scripts}, null, 2));"'
...
Run-Step -Name "Artifact freshness snapshot" -Command 'powershell -NoProfile -Command "if (Test-Path artifacts) { Get-ChildItem artifacts -File | Sort-Object LastWriteTime -Descending | Select-Object -First 40 Name,Length,LastWriteTime | Format-Table -AutoSize } else { Write-Output ''No artifacts directory.'' }"'
