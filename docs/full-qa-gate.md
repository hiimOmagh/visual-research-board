...
v1.7.0 consolidates the project validation surface into one categorized runner.
...
Before v1.7.0, `npm run qa` was a long shell chain. That made it harder to isolate failures, run one family of checks, or attach release evidence. v1.7.0 makes the validation layer inspectable without changing product behavior.

## v1.7.0 security category
...
The public-demo category verifies release-candidate copy, demo-safety docs, unavailable-provider boundaries, and preservation of v1.7.0 server-only provider key handling.
