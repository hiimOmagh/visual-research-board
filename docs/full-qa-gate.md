...
v2.1.1 consolidates the project validation surface into one categorized runner.
...
Before v2.1.1, `npm run qa` was a long shell chain. That made it harder to isolate failures, run one family of checks, or attach release evidence. v2.1.1 makes the validation layer inspectable without changing product behavior.

## v2.1.1 security category
...
The public-demo category verifies release-candidate copy, demo-safety docs, unavailable-provider boundaries, and preservation of v2.1.1 server-only provider key handling.
