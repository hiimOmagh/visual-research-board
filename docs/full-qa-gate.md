...
v0.8.3 consolidates the project validation surface into one categorized runner.
...
Before v0.8.3, `npm run qa` was a long shell chain. That made it harder to isolate failures, run one family of checks, or attach release evidence. v0.8.3 makes the validation layer inspectable without changing product behavior.

## v0.8.3 security category
...
The public-demo category verifies release-candidate copy, demo-safety docs, unavailable-provider boundaries, and preservation of v0.8.3 server-only provider key handling.
