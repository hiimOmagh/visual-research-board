...
for (const file of requiredFiles) assert(existsSync(join(root, file)), `Missing v0.7.0 file: ${file}`);
...
assert(runtime.includes('APP_VERSION = "0.7.0"'), "provider runtime report must expose app version 0.7.0");
...
for (const env of ["RIJKSMUSEUM_API_KEY", "NYPL_API_KEY", "DPLA_API_KEY"]) assert(runtimeRoute.includes(env), `provider runtime route must inspect ${env}`);
...
assert(searchPanel.includes("visual-research-board-library-v0.7.0.json"), "library export filename must use v0.7.0");

const docs = read("docs/museum-open-access-provider-pack.md");
for (const token of ["v0.7.0", "Met Museum", "Art Institute of Chicago", "Cleveland Museum", "Wellcome Collection", "Biodiversity Heritage Library", "Gallica", "NARA", "npm run museum:providers:check"]) {
...
console.log("Museum/Open-Access Provider Pack checks passed for v0.7.0.");
process.exit(0);
