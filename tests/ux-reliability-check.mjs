...
]) assert(existsSync(join(root, file)), `Missing v0.7.1 file: ${file}`);
...
for (const token of [
  ...
  "loadDemoProject",
  "useDemoSearchTopic",
  "visual-research-board-library-v0.7.1.json",
  "guided onboarding",
...
assert(providerToggle.includes("v0.7.1 keeps provider setup explicit"), "ProviderTogglePanel must mention v0.7.1 provider setup clarity");
...
assert(docs.includes("v0.7.1"), "docs must identify v0.7.1");
...
console.log("UX Reliability + Empty State Polish checks passed for v0.7.1.");
process.exit(0);
