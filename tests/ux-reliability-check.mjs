...
]) assert(existsSync(join(root, file)), `Missing v2.0.2 file: ${file}`);
...
for (const token of [
  ...
  "loadDemoProject",
  "useDemoSearchTopic",
  "visual-research-board-library-v2.0.2.json",
  "guided onboarding",
...
assert(providerToggle.includes("v2.0.2 keeps provider setup explicit"), "ProviderTogglePanel must mention v2.0.2 provider setup clarity");
...
assert(docs.includes("v2.0.2"), "docs must identify v2.0.2");
...
console.log("UX Reliability + Empty State Polish checks passed for v2.0.2.");
process.exit(0);
