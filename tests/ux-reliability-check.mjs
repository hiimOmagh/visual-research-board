...
]) assert(existsSync(join(root, file)), `Missing v1.6.0 file: ${file}`);
...
for (const token of [
  ...
  "loadDemoProject",
  "useDemoSearchTopic",
  "visual-research-board-library-v1.6.0.json",
  "guided onboarding",
...
assert(providerToggle.includes("v1.6.0 keeps provider setup explicit"), "ProviderTogglePanel must mention v1.6.0 provider setup clarity");
...
assert(docs.includes("v1.6.0"), "docs must identify v1.6.0");
...
console.log("UX Reliability + Empty State Polish checks passed for v1.6.0.");
process.exit(0);
