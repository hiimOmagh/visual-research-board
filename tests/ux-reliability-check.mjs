...
]) assert(existsSync(join(root, file)), `Missing v0.8.2 file: ${file}`);
...
for (const token of [
  ...
  "loadDemoProject",
  "useDemoSearchTopic",
  "visual-research-board-library-v0.8.2.json",
  "guided onboarding",
...
assert(providerToggle.includes("v0.8.2 keeps provider setup explicit"), "ProviderTogglePanel must mention v0.8.2 provider setup clarity");
...
assert(docs.includes("v0.8.2"), "docs must identify v0.8.2");
...
console.log("UX Reliability + Empty State Polish checks passed for v0.8.2.");
process.exit(0);
