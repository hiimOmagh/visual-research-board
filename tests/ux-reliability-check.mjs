...
]) assert(existsSync(join(root, file)), `Missing v2.1.1 file: ${file}`);
...
for (const token of [
  ...
  "loadDemoProject",
  "useDemoSearchTopic",
  "visual-research-board-library-v2.1.1.json",
  "guided onboarding",
...
assert(providerToggle.includes("v2.1.1 keeps provider setup explicit"), "ProviderTogglePanel must mention v2.1.1 provider setup clarity");
...
assert(docs.includes("v2.1.1"), "docs must identify v2.1.1");
...
console.log("UX Reliability + Empty State Polish checks passed for v2.1.1.");
process.exit(0);
