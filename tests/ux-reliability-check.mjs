...
]) assert(existsSync(join(root, file)), `Missing v2.1.7 file: ${file}`);
...
for (const token of [
  ...
  "loadDemoProject",
  "useDemoSearchTopic",
  "visual-research-board-library-v2.1.7.json",
  "guided onboarding",
...
assert(providerToggle.includes("v2.1.7 keeps provider setup explicit"), "ProviderTogglePanel must mention v2.1.7 provider setup clarity");
...
assert(docs.includes("v2.1.7"), "docs must identify v2.1.7");
...
console.log("UX Reliability + Empty State Polish checks passed for v2.1.7.");
process.exit(0);
