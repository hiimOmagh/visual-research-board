...
]) assert(existsSync(join(root, file)), `Missing v2.2.0 file: ${file}`);
...
for (const token of [
  ...
  "loadDemoProject",
  "useDemoSearchTopic",
  "visual-research-board-library-v2.2.0.json",
  "guided onboarding",
...
assert(providerToggle.includes("v2.2.0 provider setup clarity"), "ProviderTogglePanel must mention v2.2.0 provider setup clarity");
assert(read("src/components/search/ProviderTogglePanel.tsx").includes("v2.2.0 provider setup clarity"), "ProviderTogglePanel must mention v2.2.0 provider setup clarity");
...
assert(docs.includes("v2.2.0"), "docs must identify v2.2.0");
...
console.log("UX Reliability + Empty State Polish checks passed for v2.2.0.");
process.exit(0);
