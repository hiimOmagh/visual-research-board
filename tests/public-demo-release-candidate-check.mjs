...
if (!fs.existsSync(packageJsonPath)) {
  ...
  assert(packageJson.version === "2.0.1", `package.json version must be 2.0.1, got ${packageJson.version}`);
  ...
  assert(scripts["security:key:check"] === "node tests/security-key-handling-check.mjs", "v2.0.1 must preserve v2.0.1 security:key:check");
  assert(scripts["qa:security"] === "node scripts/full-qa-gate.mjs --category=security", "v2.0.1 must preserve v2.0.1 qa:security");
}
...
if (exists("scripts/full-qa-gate.mjs")) {
  ...
  assertIncludes(fullQaGate, "2.0.1", "scripts/full-qa-gate.mjs");
...
if (exists("tests/full-qa-gate-check.mjs")) {
  ...
  assertIncludes(fullQaGateCheck, "2.0.1", "tests/full-qa-gate-check.mjs");
}
...
assertIncludes(readme, "v2.0.1", "README.md");
...
console.log("Public demo release-candidate check passed for v2.0.1.");
