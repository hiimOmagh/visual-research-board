...
if (!fs.existsSync(packageJsonPath)) {
  ...
  assert(packageJson.version === "1.7.0", `package.json version must be 1.7.0, got ${packageJson.version}`);
  ...
  assert(scripts["security:key:check"] === "node tests/security-key-handling-check.mjs", "v1.7.0 must preserve v1.7.0 security:key:check");
  assert(scripts["qa:security"] === "node scripts/full-qa-gate.mjs --category=security", "v1.7.0 must preserve v1.7.0 qa:security");
}
...
if (exists("scripts/full-qa-gate.mjs")) {
  ...
  assertIncludes(fullQaGate, "1.7.0", "scripts/full-qa-gate.mjs");
...
if (exists("tests/full-qa-gate-check.mjs")) {
  ...
  assertIncludes(fullQaGateCheck, "1.7.0", "tests/full-qa-gate-check.mjs");
}
...
assertIncludes(readme, "v1.7.0", "README.md");
...
console.log("Public demo release-candidate check passed for v1.7.0.");
