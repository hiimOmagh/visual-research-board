...
assert(!lockText.includes('"is-finalizationregistry": "^2.0.2"'),"lockfile must not mutate dependency versions to app version");
assert(!lockText.includes('"which-boxed-primitive": "^2.0.2"'),"lockfile must not mutate dependency versions to app version");
...
for (const token of ["public demo UX final pass","release docs","screenshots","full QA artifact","npm ci","tag v2.0.2"]) assert(checklist.includes(token),`stable workflow checklist must include ${token}`);
for (const file of ["README.md","PATCH_MANIFEST.md","docs/release-checklist.md","docs/validation-report.md"]) assert(exists(file)&&read(file).includes("v2.0.2"),`${file} must reference v2.0.2`);
