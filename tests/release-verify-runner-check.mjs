...
if (exists(reportPath)) {
  ...
  if (report.app_version !== VERSION) {
    console.warn(`WARN release verify runner: report is ${report.app_version}, expected ${VERSION}. Run npm run verify:release to regenerate it.`);
  } else if (report.status !== "passed") {
    console.warn(`WARN release verify runner: report status is ${report.status}. Run npm run verify:release to regenerate it.`);
  }
}
