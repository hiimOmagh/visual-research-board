...
assert(!lockText.includes('"is-finalizationregistry": "^2.1.4"'), "lockfile must not mutate is-finalizationregistry dependency to app version");
assert(!lockText.includes('"which-boxed-primitive": "^2.1.4"'), "lockfile must not mutate which-boxed-primitive dependency to app version");
...
if (exists(reportPath)) {
  ...
  if (report.app_version !== VERSION) {
    console.warn(`WARN activation pack export integration: full QA artifact is ${report.app_version}, expected ${VERSION}. Run npm run qa to regenerate it.`);
  } else if (report.status !== "passed" || report.failed_gate_count !== 0) {
    console.warn(`WARN activation pack export integration: full QA artifact status is ${report.status} with failed_gate_count ${report.failed_gate_count}. Run npm run qa to regenerate it.`);
  }
}
