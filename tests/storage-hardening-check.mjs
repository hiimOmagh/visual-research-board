...
]) assert(existsSync(join(root, file)), `Missing v0.6.0 file: ${file}`);
...
for (const token of [
  ...
  "visual-research-board-backup-v0.6.0.json",
  "visual-research-board-library-v0.6.0.json",
  "Validation:",
...
assert(manifest.includes("v0.6.0"), "PATCH_MANIFEST must identify v0.6.0");
assert(manifest.includes("Local Storage + Import/Export Hardening"), "PATCH_MANIFEST must identify the feature");

const lock = read("package-lock.json");
assert(lock.includes('"version": "0.6.0"'), "package-lock.json must use version 0.6.0");
...
console.log("Local Storage + Import/Export Hardening checks passed for v0.6.0.");
process.exit(0);
