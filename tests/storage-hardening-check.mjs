...
assert(pkg.scripts?.qa?.includes("storage-hardening-check"), "npm run qa must include storage-hardening-check");
...
]) assert(existsSync(join(root, file)), `Missing v0.5.1 file: ${file}`);
...
for (const token of [
  ...
  "visual-research-board-backup-v0.5.1.json",
  "visual-research-board-library-v0.5.1.json",
  "Validation:",
...
assert(manifest.includes("v0.5.1"), "PATCH_MANIFEST must identify v0.5.1");
assert(manifest.includes("Local Storage + Import/Export Hardening"), "PATCH_MANIFEST must identify the feature");

const lock = read("package-lock.json");
assert(lock.includes('"version": "0.5.1"'), "package-lock.json must use version 0.5.1");
...
console.log("Local Storage + Import/Export Hardening checks passed for v0.5.1.");
process.exit(0);
