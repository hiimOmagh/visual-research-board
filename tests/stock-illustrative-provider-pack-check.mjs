...
for (const file of requiredFiles) assert(existsSync(join(root, file)), `Missing v2.1.4 file: ${file}`);
...
assert(runtime.includes('APP_VERSION = "2.1.4"'), "provider runtime report must expose app version 2.1.4");
...
assert(searchPanel.includes("v2.1.4"), "SearchPanel header must show v2.1.4");
assert(searchPanel.includes("visual-research-board-library-v2.1.4.json"), "library export filename must use v2.1.4");
assert(searchPanel.includes("visual-research-board-backup-v2.1.4.json"), "backup export filename must use v2.1.4");

const docs = read("docs/stock-illustrative-provider-pack.md");
for (const token of ["v2.1.4", "Pixabay", "Pexels", "Unsplash", "stock_illustrative", "npm run stock:providers:check"]) {
...
console.log("Stock/Illustrative Provider Pack checks passed for v2.1.4.");
process.exit(0);
