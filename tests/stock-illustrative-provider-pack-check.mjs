...
for (const file of requiredFiles) assert(existsSync(join(root, file)), `Missing v2.0.3 file: ${file}`);
...
assert(runtime.includes('APP_VERSION = "2.0.3"'), "provider runtime report must expose app version 2.0.3");
...
assert(searchPanel.includes("v2.0.3"), "SearchPanel header must show v2.0.3");
assert(searchPanel.includes("visual-research-board-library-v2.0.3.json"), "library export filename must use v2.0.3");
assert(searchPanel.includes("visual-research-board-backup-v2.0.3.json"), "backup export filename must use v2.0.3");

const docs = read("docs/stock-illustrative-provider-pack.md");
for (const token of ["v2.0.3", "Pixabay", "Pexels", "Unsplash", "stock_illustrative", "npm run stock:providers:check"]) {
...
console.log("Stock/Illustrative Provider Pack checks passed for v2.0.3.");
process.exit(0);
