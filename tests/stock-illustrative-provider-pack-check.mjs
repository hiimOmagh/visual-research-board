...
for (const file of requiredFiles) assert(existsSync(join(root, file)), `Missing v0.9.0 file: ${file}`);
...
assert(runtime.includes('APP_VERSION = "0.9.0"'), "provider runtime report must expose app version 0.9.0");
...
assert(searchPanel.includes("v0.9.0"), "SearchPanel header must show v0.9.0");
assert(searchPanel.includes("visual-research-board-library-v0.9.0.json"), "library export filename must use v0.9.0");
assert(searchPanel.includes("visual-research-board-backup-v0.9.0.json"), "backup export filename must use v0.9.0");

const docs = read("docs/stock-illustrative-provider-pack.md");
for (const token of ["v0.9.0", "Pixabay", "Pexels", "Unsplash", "stock_illustrative", "npm run stock:providers:check"]) {
...
console.log("Stock/Illustrative Provider Pack checks passed for v0.9.0.");
process.exit(0);
