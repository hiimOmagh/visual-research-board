...
assert(pkg.scripts?.qa?.includes("stock-illustrative-provider-pack-check"), "npm run qa must include stock-illustrative-provider-pack-check");
...
assert(searchPanel.includes("visual-research-board-backup-v0.6.1.json"), "backup export filename must use v0.6.1");

const docs = read("docs/stock-illustrative-provider-pack.md");
for (const token of ["v0.6.1", "Pixabay", "Pexels", "Unsplash", "stock_illustrative", "npm run stock:providers:check"]) {
...
console.log("Stock/Illustrative Provider Pack checks passed for v0.6.1.");
process.exit(0);
