...
for (const file of requiredFiles) assert(existsSync(join(root, file)), `Missing v2.1.7 file: ${file}`);
...
assert(runtime.includes('APP_VERSION = "2.1.7"'), "provider runtime report must expose app version 2.1.7");
...
assert(searchPanel.includes("visual-research-board-backup-v2.1.7.json"), "backup export filename must use v2.1.7");

const docs = read("docs/stock-illustrative-provider-pack.md");
for (const token of ["v2.1.7", "Pixabay", "Pexels", "Unsplash", "stock_illustrative", "npm run stock:providers:check"]) {
...
console.log("Stock/Illustrative Provider Pack checks passed for v2.1.7.");
process.exit(0);
