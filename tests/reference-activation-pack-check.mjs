...
if (exists(reportPath)) {
  ...
  if (report.app_version !== VERSION) {
    if (!suppressStaleReportWarnings) console.warn(`WARN reference activation pack: full QA artifact is ${report.app_version}, expected ${VERSION}. Run npm run qa to regenerate it.`);
  } else if (report.status !== "passed" || report.failed_gate_count !== 0) {
    if (!suppressStaleReportWarnings) console.warn(`WARN reference activation pack: full QA artifact status is ${report.status} with failed_gate_count ${report.failed_gate_count}. Run npm run qa to regenerate it.`);
  }
}
